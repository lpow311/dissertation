...
Thank you to the Computer Science Faculty at the University of Bath and Andreas Theophilou for his guidance during this dissertation and helping shape the project from the initial proposal to this final submission. I'd also like to thank everyone else who has helped me along this journey.

I acknowledge that this content, analysis and intepretation is my own, however I it is worth noting that I did use Claude (Anthropic, https://claude.ai/) to assist with proof reading final sections and summarising initial notes and investigations.

\newpage

\chapter*{}
This research has been conducted in line with the University of Bath’s ethics policy and ethical approval has been obtained through Ethics@bath: 12399
...
\begin{description}
\item[RQ1]Implement and compare the global optimised genetic algorithm (GA) against an individually optimised greedy routing strategy across structurally different topologies and seeds to understand if network topology affects the performance of each approach.
...
\item[RQ3]Determine the affect of human characteristics on evacuation outcomes by embedding behaviours into the GA fitness function. Specifically:
\begin{description}
  \item[RQ3a]Evaluate how physical limitations such as walking speed affect algorithm performance such as total evacuation time and congestion.
  \item[RQ3b]Evaluate how delayed starts affect relative algorithm performance, in particular the congestion metric.
  \item[RQ3c]Evaluate how compliance levels across the population affect the GA performance, where non-compliance agents follow a shortest path route.
...
\item Three structurally different network topologies, aiming to reflect the different situations evacuations which occur, as described above.
\item Congestion modelling via node capacity constraints with queue-based delay.
...
This gap between individual versus the collective optimal is defined as the Price of Anarchy (PoA)\citep{PAPADIMITRIOU_2001_POA}. PoA is a concept utilised in game theory \citep{COMINETTI_2024_POA} which defines the ratio between the worst case scenario and the best, quantifying the inefficiency of selfish behaviour on the system \citep{ROUGHGARDEN_2015_POA}. \cite{ROUGHGARDEN_2003_POA} showed that the PoA effects arise even in very small networks, meaning scalable models of city-wide evacuations can be informed by simpler examples. \cite{FRIES_2022_POA} examined planned evacuation routes versus guidance from navigation systems (shortest path) and found the planned routes produces less congestion. These results reinforce that evacuation routing strategies must explicitly manage the trade-off between individual convenience and system wide efficiency.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{dissertation_images/architecture_diagram_design.png}
...
\begin{table}[htb]
  \bigskip
  \begin{center}
    \begin{tabular}{| >{\raggedright\arraybackslash}p{3cm} | p{5cm} | p{6.5cm} |}
      \hline
...
\begin{itemize}
  ...
  \item Start and exit locations are fixed so performance differences come from the routing strategy not the change in setup.
  \item Paths are loop-free to avoid unnecessary cycling.
\end{itemize}
...
The simulation framework was implemented in Python, chosen for the wide availability of libraries such as NetworkX, NumPy and JAX. NetworkX provides the ability to construct graph networks and has unbuilt graphical functions such as shortest path. NumPy and JAX were used throughout the code for numerical operators and seed generation. The code base is structured around eight core classes in \autoref{fig:code-classes}. A link to the full code base and user guide can be found in \autoref{chap:code}. This chapter documents the key implementation decisions made during development, testing and validation prior to running the main experiments.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.32\textwidth]{dissertation_images/grid-like_topology.png}
    \hfill
    \includegraphics[width=0.32\textwidth]{dissertation_images/moderate-complexity_topology.png}
    \hfill
    \includegraphics[width=0.32\textwidth]{dissertation_images/bottleneck-constrained_topology.png}
...
\begin{figure}[h]
    ...
    \caption[Analysis number of agents for genetic algorithm]{From left to right, Grid, Moderate and Bottleneck Topologies number of agents vs Fitness Function for genetic algorithm, with bands showing the values across the 10 seeds.}
...
\begin{figure}[h]
    ...
    \caption[Analysis number of agents for greeedy algorithm]{From left to right, Grid, Moderate and Bottleneck Topologies number of agents vs Fitness Function for greedy algorithm, with bands showing the values across the 10 seeds.}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{dissertation_images/route_comparison_grid.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{dissertation_images/route_comparison_moderate.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{dissertation_images/route_comparison_bottleneck.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{dissertation_images/walking_speed_breakdown.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{dissertation_images/walking_speed_dispersion.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{dissertation_images/delay_start_breakdown.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{dissertation_images/full-behaviour-dispersion-N3.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{dissertation_images/full-behaviour-dispersion-N5.png}
...
In answer to RQ3a, physical limitations in the form of walking speed do affect the algorithm performance with the GA gaining an advantage in the bottleneck topology by exploiting speed in the fitness function. For RQ3b, delayed starts have minimal (if not no) impact on the performance of either algorithm, suggesting pre-movement delay is less critical to routing strategy.
...
\vfill
Number of words until this point, excluding front matter: 12061.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Code and User Documentation}
\label{chap:code}
