...
class Mutation:
    ...
    def partial_path_mutation(self, children: Chromosome) -> Chromosome:
        mutated_children = []
        for child_chromosome in children:
            for agent_name, agent in child_chromosome.agents.items():
                ...
                agent_mutation_prob = random.uniform(key=mutation_sub_key, shape=(), minval=0, maxval=1)
    ...
    def extract_path_points(self, agent) -> tuple:
        ...
        point2 = random.randint(key=sub_key_point2, shape=(), minval=max_point, maxval=len(agent_path))
