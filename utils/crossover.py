...
class CrossOver:
    ...
    def single_point_crossover(self, parent1, parent2, key) -> list:
        ...
        key, sub_key = random.split(key=key)
        crossover_point = random.randint(key=sub_key, shape=(), minval=1, maxval=self.num_agents-1)
        ...
        children = [
            Chromosome(agents=child1, seed=child1_key),
            Chromosome(agents=child2, seed=child2_key)    
        ]
