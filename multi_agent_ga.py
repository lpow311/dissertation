...
def simulate_evacuation(city: Environment, num_agents: int, population_size: int, algorithm_seed: int):
    ...
    ga = GeneticAlgorithm(
        exit_criteria=exit_criteria, pop_size=population_size, seed=algorithm_seed, num_agents=num_agents
    )
    ...
    while not terminate:
        ...
        population = ga.survivor_selection(children=mutated_children, old_population=population, keep_best=True)
        ...
        print(f"Evolution {evolution:4d} | best: {best:.3f} | avg: {avg:.3f} | min: {worst:.3f} | avg path length={avg_path_length:.3f} | pop={len(population)}")
